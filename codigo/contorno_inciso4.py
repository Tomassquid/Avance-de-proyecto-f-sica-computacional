...
def mapa_contorno():
    ...
    for i, dm2 in enumerate(rango_deltam21):
        for j, t12 in enumerate(rango_theta):
            ...
            # paso 3: extraemos el estado final en la superficie
            psi_f = psi_historial[-1]
            
            #tomamos el angulo theta de la iteración actual directamente de la lista
            theta_actual = rango_theta[j]
            
            #paso 4: calculamos U con el theta_12 dinamico de esta iteracion
            U_dinamico = matriz_pmns(theta_actual, theta_13, theta_23, delta_cp)
            U_daga = np.conjugate(U_dinamico).T
            
            #paso 5: proyectamos a masa y calculamos decoherencia hacia la Tierra
            psi_masa = np.dot(U_daga, psi_f)
            prob_e = (np.abs(psi_masa[0])**2 * np.abs(U_dinamico[0,0])**2 + 
                      np.abs(psi_masa[1])**2 * np.abs(U_dinamico[0,1])**2 + 
                      np.abs(psi_masa[2])**2 * np.abs(U_dinamico[0,2])**2)
